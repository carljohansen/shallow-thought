...
export class ComputerPlayer {
  ...
  //  @Output() calculationProgress: EventEmitter<number> = new EventEmitter();
    ...
    public getMovesAndReplies(board: Chess.Board, headStartMoves: Chess.GameMove[]): MoveWithReplies[] {
        ...
        const playerMoves = initialMoves.map(move => { return <MoveWithReplies>{ move: move, replies: null }; });
    ...
    private getBestMoveMinimax(board: Chess.Board, headStartMoves: Chess.GameMove[], depth: number, isCaptureChain: boolean, betaScore: number): EvaluatedMove {
        ...
        for (let move of movesAndReplies) {
            ...
            if (!bestMoveSoFar
                ...
                //if ((depth===0) && (move.move.fromSquare.algebraicNotation === "d7"
                //    || move.move.fromSquare.algebraicNotation === "f7")) {
                //    debugger;
                //}
            }
